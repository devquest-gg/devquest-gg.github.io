...
(function (w) {
  ...
  function suggestRowHTML(kind, r) {
    ...
    var un = r[3];
    var phref = un ? ('/credits/person.html?name=' + encodeURIComponent(r[1])) : ('/portfolio/' + encodeURIComponent(r[0]));
  ...
  function openClaim(opts) {
    ...
    function promptTeammates(gameSlug, gameTitle, studio, personSlug){
      ...
      function row(i){
        return '<div class="dq-row2" style="gap:8px;margin-bottom:8px">'+
          '<div style="flex:2"><input class="dq-tm-name" data-i="'+i+'" autocomplete="off" placeholder="Teammate\'s name"></div>'+
          '<div style="flex:1"><input class="dq-tm-role" data-i="'+i+'" autocomplete="off" placeholder="Role (optional)"></div>'+
        '</div>';
      }
      modal.innerHTML = '<button class="dq-x" aria-label="Close">×</button>'+
        '<div class="dq-mh">Who did you work with?</div>'+
        '<div class="dq-sub">Name up to 3 people you shipped <b>'+esc(gameTitle||"this game")+'</b> with. They\'ll find themselves already listed when they search their name, and can confirm your credit too. No emails are sent.</div>'+
        row(0)+row(1)+row(2)+
        '<div class="dq-actions"><button class="dq-cancel dq-tm-skip">Skip</button><button class="dq-submit dq-tm-save">Add teammates →</button></div>'+
        '<div class="dq-foot">Optional — naming teammates is what helps the site spread to the people you shipped with.</div>';
      modal.querySelector(".dq-x").onclick = function(){ close(); w.location.href = dest; };
      modal.querySelector(".dq-tm-skip").onclick = function(){ close(); w.location.href = dest; };
      modal.querySelector(".dq-tm-save").onclick = function(){
        var btn = this;
        var nameEls = modal.querySelectorAll(".dq-tm-name"), roleEls = modal.querySelectorAll(".dq-tm-role");
        var jobs = [];
        for(var i=0;i<nameEls.length;i++){ var nm = nameEls[i].value.trim(); if(nm) jobs.push({ name:nm, role:(roleEls[i]&&roleEls[i].value.trim())||null }); }
        if(!jobs.length){ close(); w.location.href = dest; return; }
        btn.disabled = true; btn.textContent = "Adding…";
        var done = 0;
        jobs.forEach(function(j){
          w.DQAPI.attributeTeammate({ game_slug: gameSlug, game_title: gameTitle, studio: studio||null, name: j.name, role: j.role }).catch(function(){}).then(function(){
            done++; if(done===jobs.length){ close(); w.location.href = dest; }
          });
        });
      };
    }
      ...
      if (w.DQAPI) {
        ...
        w.DQAPI.createCredit(payload()).then(function (r) {
          if (r.status === 201 || r.ok) {
            var slug = r.data && r.data.person_slug;
            var gSlug = (r.data && r.data.game_slug) || opts.game_slug || "";
            var gTitle = opts.game_title || (isAdd ? val("dqc-gtitle") : "");
            var gStudio = (isGame ? (opts.studio || null) : (isAdd ? val("dqc-gstudio") : null)) || null;
            promptTeammates(gSlug, gTitle, gStudio, slug);
