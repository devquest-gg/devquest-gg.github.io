...
(function () {
  ...
  function dayOfYear(dt) {
    return Math.floor((Date.UTC(dt.getUTCFullYear(), dt.getUTCMonth(), dt.getUTCDate())
      - Date.UTC(dt.getUTCFullYear(), 0, 0)) / 86400000);
  }
  function mdKey(dt) {
  ...
  function buildPopover(state) {
    ...
    if (state.type === "claimed") {
      ...
      inner =
        ...
        capNote +
        '<div class="dqd-btns">' +
        '<a class="dqd-btn pri" href="' + CONFIG.claimPage + '?slot=' + state.slot + '">Claim this slot →</a>' +
